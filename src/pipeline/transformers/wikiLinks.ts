...
export function resolveWikiLinkMarkers(
  ...
  return content.replace(UNRESOLVED_MARKER_RE, (_match, target: string, display: string) => {
...
export const WikiLinkTransformer: TransformerPlugin<WikiLinkOptions> = (userOpts) => {
  ...
  return {
    ...
    transform(ctx: TransformContext): TransformContext {
      return {
        ...ctx,
        content: replaceWikiLinks(
          ctx.content,
          opts.resolve,
          opts.outlineUrl,
          opts.preserveUnresolved
        ),
      };
    },
  };
